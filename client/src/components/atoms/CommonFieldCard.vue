...
<template>
  <div v-if="!requestedButEmpty" :class="[
    ...
    <div class="flex items-center gap-2 mb-2">
      ...
      <button class="ml-auto p-1 rounded hover:bg-white/10 transition-colors" @click="$emit('toggle-view')"
        :class="{ 'text-red-400': inFields }">
        <Eye class="w-4 h-4" />
      </button>
      <button class="p-1 rounded hover:bg-white/10 transition-colors" @click="$emit('toggle-array')"
    ...
    <div class="text-lg font-medium text-white">
      ...
      <template v-else>
        <div class="text-white/40 text-sm italic flex items-center gap-2">
          <Eye class="w-4 h-4" />
